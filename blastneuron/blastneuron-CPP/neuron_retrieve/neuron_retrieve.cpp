...
bool neuron_retrieve(NeuronTree query, QList<double*> & feature_list, vector<V3DLONG>  & result, V3DLONG cand, int method_code, int norm_code)
{
	...
	//prune short branch for the query neuron
	NeuronTree query_pruned;
	if (!prune_branch(query, query_pruned))
	{
	...
	if (method_code==1)
		computeFeature(query_pruned, qf);
	else if (method_code==2)
		computeGMI(query_pruned, qf);
	...
	switch (norm_code)
	{
		case 1:
			...
			break;
		case 2:
			if (!unitVar(qf, feature_list, cand, sbj, VSIZE, score))
			{
				printf("Error in unitVar.\n");
				if (qf) {delete []qf; qf=NULL;}
				for (V3DLONG i=0;i<neuronNum;i++)
					if (feature_list[i]) {delete []feature_list[i]; feature_list[i]=NULL;}
				if (sbj) {delete []sbj; sbj=NULL;}
